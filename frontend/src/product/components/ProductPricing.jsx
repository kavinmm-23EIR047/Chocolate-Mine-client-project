  ...
  return (
    <>
      <div className="space-y-5 sm:space-y-6">
        ...
        <div className="bg-card rounded-2xl sm:rounded-3xl p-5 sm:p-6 space-y-3.5 text-sm font-bold border-2 border-border/60 shadow-xs">
          <div className="flex justify-between text-heading/80 uppercase text-[11px] sm:text-xs tracking-widest gap-2 font-black">
            <span>Product Base Price</span>
            <span className="text-right text-heading font-black">{formatCurrency(productBaseOnly > 0 ? productBaseOnly : (finalPrice - addonSum))}</span>
          </div>
          {flavorPrice > 0 && (
            <div className="flex justify-between text-primary uppercase text-[11px] sm:text-xs tracking-widest gap-2 font-black">
              <span>Flavor: {selectedFlavor?.name}</span>
              <span className="text-right font-black">+ {formatCurrency(flavorPrice)}</span>
            </div>
          )}
          {addonSum > 0 && (
            <div className="flex justify-between text-primary uppercase text-[11px] sm:text-xs tracking-widest gap-2 font-black">
              <span>Add-ons Total</span>
              <span className="text-right font-black">+ {formatCurrency(addonSum)}</span>
            </div>
          )}
          {offerDiscount > 0 && (
            <div className="flex justify-between text-success-text uppercase text-[11px] sm:text-xs tracking-widest gap-2 font-black">
              <span>Offer Savings</span>
              <span className="text-right font-black">- {formatCurrency(offerDiscount)}</span>
            </div>
          )}
          {couponSavings > 0 && (
            <div className="flex justify-between text-success-text uppercase text-[11px] sm:text-xs tracking-widest gap-2 font-black">
              <span>Coupon Discount ({product?.coupon?.code})</span>
              <span className="text-right font-black">- {formatCurrency(couponSavings)}</span>
            </div>
          )}
          <div className="border-t border-border/60 pt-3 flex items-center justify-between font-black text-heading text-base sm:text-lg tracking-tight">
            <span className="uppercase text-[11px] sm:text-xs tracking-widest text-heading/80">Final Total Price</span>
            <span className="text-heading font-black">{formatCurrency(finalPrice)}</span>
          </div>
        </div>
      </div>
